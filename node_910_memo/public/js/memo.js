document.addEventListener("DOMContentLoaded", () => {
  ...
  이때 한 개의 메모를 구성하는 요소 들은 li, img, span 로 되어 있다
  어떤 부분을 클릭했을때 메모의 seq 를 알려줄지 만들기 위해서
  여러가지 tag 에 event 를 설정해야 한다

  하지만 그러기에는 event 코드가 너무 복잡해질 수 있다.
  ...
  3. memo 가 포함된 target 을 알게되면
    ...
    3-5. target.closest("LI") 를 실행하여 부모 LI tag 로 부터 dataset.seq 를 
      getter 한다.
  ...
  */
  ...
  memo_box.addEventListener("click", async (e) => {
    ...
    if (classList.contains("memo")) {
      ...
      // 입력된 메모를 클릭했을때 시간,날짜,제목,내용이 입력칸에 뜬다

      // 서버에서 받은 메모데이터를
      // 각각의 input tag 에 value 값에 setting 하여
      // 수정데이터가 보이도록 하기
      ...
      // 이미지에 메모의 이미지를 세팅하기
      memo_image.src = `/images/${json.m_image}`; // ajax 방식

      // btn_save 는 input tag 를 사용한 button 이므로
      // value 속성을 변경하면 화면에 보이는 text 가 변경된다
      btn_save.value = "수정";
      btn_save.classList.add("update"); // 업데이트 클래스추가
